...
- {:.text-success}优势：易理解、生活常用，易测量（利用液柱温度计）
- {:.text-danger}缺陷：
    - 局限性：受温度计材料、工作物质性质限制
    - 随意性：不同温标定义不同
    - 线性内插：液柱长度与温度关系非线性，应用范围窄
  {:.text-danger}
-  {:.text-info}实验上可行，常作为实际使用的温标（或其一部分）
...
- {:.text-success}优势：不依赖于任何物质特性（可逆热机效率仅与温度有关），绝对可靠
- {:.text-danger}缺陷：真实世界不存在可逆热机，测量过程无法实现；热量的测量依赖于温度测量；仅具有理论意义
...
- 定容测量较容易实现，精度更高
- {:.text-success}优势：实际可测，可看作热力学温标的一种实现方式
- {:.text-danger}缺陷：设备复杂；实际气体有偏离，需反复修正，使用不便
- {:.text-info}实现设备：氢气温度计、氦气温度计
